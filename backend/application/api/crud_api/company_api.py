...
class Recruit(Resource):
    ...
    def post(self):
        ...
        application_id = post_cred.get('application_id')
        if not application_id:
            return make_response(jsonify({'message': 'application_id is required'}), 400)
        ...
        if drive.company_id != Company.query.filter_by(user_id=current_user.id).first().id:
            return make_response(jsonify({'message': 'Not authorized'}), 403)
        
        student = Student.query.get(application.student_id)
        if not student.available:
            return make_response(jsonify({'message': 'Student already recruited'}), 409)
        
        existing_request = RecruitmentRequest.query.filter(
            RecruitmentRequest.student_id == student.id,
            RecruitmentRequest.status.in_(['pending', 'confirmed'])
        ).first()
        
        if existing_request:
            return make_response(jsonify({'message': 'Student already has a pending/confirmed recruitment'}), 409)
        ...
        db.session.add(recruitment_request)
        db.session.commit()
        
        return make_response(jsonify({'message': 'Recruitment request sent'}), 200)
